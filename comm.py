...
# Function to set up server and listen for connections
def setup_server():
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # Allow reuse of the socket
...
# Function to wait for button release
def wait_for_release(button_pin):
    while GPIO.input(button_pin) == GPIO.LOW:
        sleep(0.01)

# Function to choose a name based on weighted calls
def choose_name():
    ...
    while True:
        ...
        while True:
            ...
            if GPIO.input(BUTTON_PIN_1) == GPIO.LOW:
                ...
                try:
                    client_socket.sendall(message.encode(1024))
